...
- (void)spawnObjects:(NSString*)setName lName:(NSString*)levelName{
    ...
    for(int i=0;i<tempLevel.count;i++){
        ...
        if(r==0){
            ...
            ob =[MovingObject spriteNodeWithImageNamed:@"Triangle"];
            CGMutablePathRef trianglePath = CGPathCreateMutable();
            CGPathMoveToPoint(trianglePath, nil, -ob.size.width/2, -ob.size.height/2);
            CGPathAddLineToPoint(trianglePath, nil,ob.size.width/2, -ob.size.height/2);
            CGPathAddLineToPoint(trianglePath, nil, 0,ob.size.height/2);
            CGPathAddLineToPoint(trianglePath, nil, -ob.size.width/2, -ob.size.height/2);
            ob.physicsBody = [SKPhysicsBody bodyWithPolygonFromPath:trianglePath];
